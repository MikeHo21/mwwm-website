...
export default function Home() {
  ...
  const content = {
    ...
    en: {
      ...
      p1Link: 'View Eco Products Details',
      ...
      p2Link: 'View Hotel Products Details',
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans">
      ...
      <header className="sticky top-0 z-50 bg-slate-950/90 backdrop-blur-md border-b border-slate-800">
        <div className="max-w-6xl mx-auto px-6 h-32 flex justify-between items-center">
          ...
          <nav className="hidden md:flex space-x-6 font-medium text-slate-300 items-center">
            ...
            >
              <button className="flex items-center gap-1 hover:text-indigo-400 transition py-2">
                {t.navProducts}
              ...
              {dropdownOpen && (
                <div className="absolute top-full left-0 w-72 bg-slate-950 border border-slate-800 rounded-xl shadow-2xl py-2 mt-1 z-50">
                  <Link 
                    href="/products/eco-hygiene" 
                    className="block px-4 py-3 text-sm text-slate-300 hover:bg-indigo-600/20 hover:text-indigo-400 transition"
                  >
                    {t.navEco}
                  </Link>
                  <Link 
                    href="/products/hotel-custom" 
                    className="block px-4 py-3 text-sm text-slate-300 hover:bg-indigo-600/20 hover:text-indigo-400 transition"
                  >
                    {t.navHotel}
                  </Link>
            ...
            <div className="flex items-center space-x-1 bg-slate-900 p-1 rounded-lg border border-slate-700 text-xs font-semibold">
              <button onClick={() => setLang('tc')} className={`px-2.5 py-1 rounded-md transition ${lang === 'tc' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}>繁</button>
              <button onClick={() => setLang('sc')} className={`px-2.5 py-1 rounded-md transition ${lang === 'sc' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}>简</button>
              <button onClick={() => setLang('en')} className={`px-2.5 py-1 rounded-md transition ${lang === 'en' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}>EN</button>
            </div>
          </nav>

          <a href="#contact" className="bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2.5 rounded-lg font-medium transition shadow-md">
            {t.cta}
          </a>
        </div>
      </header>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20 md:py-28 text-center">
        ...
        <div className="flex justify-center gap-4">
          <Link href="/products/eco-hygiene" className="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-3.5 rounded-lg font-semibold text-lg transition shadow-md">
            {t.explore}
          </Link>
      ...
      <section id="products" className="bg-slate-950 py-20 border-t border-b border-slate-800">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {/* 環保產品卡片 */}
            <div className="p-8 border border-slate-800 rounded-2xl bg-slate-900/80 hover:border-indigo-500/50 transition flex flex-col justify-between">
              <div>
                ...
                <ul className="space-y-2 border-t border-slate-800 pt-4 text-slate-200 font-medium mb-8">
              ...
              <Link href="/products/eco-hygiene" className="inline-flex items-center gap-2 text-indigo-400 font-semibold hover:underline">
                {t.p1Link} &rarr;
              </Link>
            </div>
            ...
            <div className="p-8 border border-slate-800 rounded-2xl bg-slate-900/80 hover:border-indigo-500/50 transition flex flex-col justify-between">
              <div>
                ...
                <p className="text-slate-300 leading-relaxed mb-8">{t.p2Desc}</p>
              </div>
              <Link href="/products/hotel-custom" className="inline-flex items-center gap-2 text-indigo-400 font-semibold hover:underline">
                {t.p2Link} &rarr;
              </Link>
            </div>
          </div>
      ...
      <section id="about" className="py-20 max-w-6xl mx-auto px-6">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-slate-300 mb-6 leading-relaxed">{t.aboutText}</p>
      ...
      <section id="partners" className="bg-slate-950 py-20 border-t border-slate-800">
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-3xl mx-auto">
            <div className="bg-slate-900 p-8 rounded-2xl border border-slate-800 flex flex-col items-center justify-center">
              ...
              <a href="https://www.orkin.com.hk" target="_blank" rel="noopener noreferrer" className="text-indigo-400 font-semibold hover:underline flex items-center gap-1">
          ...
          <form className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <input type="text" className="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-lg text-white focus:outline-none focus:border-indigo-500" />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea rows={4} className="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-lg text-white focus:outline-none focus:border-indigo-500"></textarea>
            </div>
